...
| Account deletion | `supabase/functions/delete-account` (auth-user delete + avatar cleanup; DB rows cascade). Prod ALSO still serves the legacy typo slug `delete-acount` (same code, deployed 2025; source was never in-repo until 2026-08-07) — `app/(tabs)/profile.tsx` tries correct name then typo; retire the typo deploy + fallback once its logs stay quiet |
