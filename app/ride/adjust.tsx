...
import { qualifierLabel, symptomLabelFor, type LoggedSymptom, type SymptomLevel } from "../../lib/rideSymptoms";
...
export default function RideAdjustScreen() {
  ...
  const forLine = headSymptom ? `For ${symptomLabelFor(headSymptom, discipline).toLowerCase()}${headQualifier ? ` on ${(qualifierLabel(headQualifier) ?? headQualifier).toLowerCase()}` : ""}` : freeText.trim() ? "For what you wrote" : "By hand";
