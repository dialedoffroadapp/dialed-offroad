...
**Provider recommendation, revised 2026-09-07 (pending River's yes): Resend over Loops.** The 2026-09-07 research report found Resend's documentation covers Apple's Hide My Email private relay (mail to an `@privaterelay.appleid.com` address is delivered only when the sending domain and each From address are registered with Apple's Private Email Relay service, with SPF and DKIM in place); no Loops documentation of the relay was found. Every Sign in with Apple account can be a relay address, so the provider that documents the relay is the safer default. What Loops would have owned (delays, sequences, unsubscribes) moves into the `lifecycle-events` function's cron leg plus a `lifecycle_sends` row per send, which the two time-based legs needed anyway; Resend also carries Supabase Auth's transactional mail later, one provider instead of two. The "Why Loops" paragraph above stays as the record of the earlier reasoning. Nothing is wired to either provider today; no key is set; switching is the forwarder in `supabase/functions/lifecycle-events` and the key name. River's checklist before any send: register the sending domain and the From address with Apple's Private Email Relay (Apple Developer, Certificates, Identifiers and Profiles, Services, Sign in with Apple for Email Communication), publish SPF and DKIM for that domain, then send one test to a relay address.
