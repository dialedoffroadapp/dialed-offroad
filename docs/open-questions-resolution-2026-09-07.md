...
- **Changed (later the same day):** the measure-sag walkthrough in the design queue is SUPERSEDED by the sag page, `app/garage/[bikeId]/sag.tsx` (bike page row, reveal link, ride-mode recheck card), which implements the three-measurement method below. Each measurement is a `sag_measurements` row linked to the setup version it was taken on; `setup_versions` rows stay immutable (River, 2026-09-07: a measurement is a fact about the bike, not a setting change, and the n=1 framing and the version trigger depend on versions never changing). Display, the meter and the ride-mode recheck read the latest row. **Deprecation:** `setup_versions.sag_measured` (boolean, v2.4.0) only ever meant "the rider measured it"; the sag page does not write it and nobody should read `sag_measured = true` as a value. The measurement itself is in `sag_measurements.riding_mm` / `static_mm`. The earlier design note, per River's item 12: three measurements, store riding sag as `sag_measured` and static sag when all three are taken; the preload-versus-spring rule becomes a roadmap note family ("static right, riding out of range: spring, not preload"). The catalog now holds `stock_static_sag_mm` per row for the comparison.
